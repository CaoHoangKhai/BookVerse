...
<input type="text" id="searchBookAuthor" class="form-control me-2 mb-4" placeholder="Tìm kiếm sách ..."
    onkeyup="searchBooks()">
...
<script>
    function searchBooks() {
        var input, filter, bookList, bookItems, bookTitle, bookDescription, i;
        input = document.getElementById('searchBookAuthor');
        filter = input.value.trim().toLowerCase(); // Loại bỏ khoảng trắng ở đầu và cuối chuỗi

        bookList = document.getElementById("bookList");
        bookItems = bookList.getElementsByClassName('book-item');

        // Nếu ô tìm kiếm trống, hiển thị tất cả sách
        if (filter === "") {
            for (i = 0; i < bookItems.length; i++) {
                bookItems[i].style.display = ""; // Hiển thị tất cả sách
            }
        } else {
            // Kiểm tra từng sách với từ khóa tìm kiếm
            for (i = 0; i < bookItems.length; i++) {
                // Lấy tên và mô tả sách từ các thuộc tính data-title và data-description
                bookTitle = bookItems[i].getAttribute('data-title');
                bookDescription = bookItems[i].getAttribute('data-description');

                // Kiểm tra nếu từ khóa tìm kiếm xuất hiện trong tên hoặc mô tả
                if (bookTitle.indexOf(filter) > -1 || bookDescription.indexOf(filter) > -1) {
                    bookItems[i].style.display = ""; // Hiển thị sách nếu tìm thấy
                } else {
                    bookItems[i].style.display = "none"; // Ẩn sách nếu không tìm thấy
                }
            }
        }
    }
